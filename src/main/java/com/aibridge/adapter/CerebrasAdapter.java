...
@ApplicationScoped
public class CerebrasAdapter implements LlmProviderAdapter {
    ...
    public ChatCompletionResponse complete(ChatCompletionRequest request, LlmConfig config) {
        ...
        try {
            ...
            if (status == 429) {
                throw new ProviderRateLimitException("Cerebras returned 429");
            }
            if (status >= 500) {
                throw new ProviderUnavailableException("Cerebras returned HTTP " + status);
            }
            ...
            throw new ProviderUnavailableException("Cerebras request failed", e);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new ProviderUnavailableException("Cerebras request interrupted", e);
        }
    }
    ...
    public Stream<ChatCompletionChunk> completeStream(ChatCompletionRequest request, LlmConfig config) {
        ...
        try {
            ...
            throw new ProviderUnavailableException("Cerebras stream failed", e);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new ProviderUnavailableException("Cerebras stream interrupted", e);
        }
    }
